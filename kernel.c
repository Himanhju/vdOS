...
byte strncmp(const char* str1, const char* str2, short n);
...
void start(){
    ...
    while(1){
        pstr(">>> ");
        for(short i = 0; i < (unsigned short)sizeof(input);i++){
            input[i] = '\0';
        }
        get_str(256, input);
        newline();
        if(strncmp("halt", input, 4) == 0){
            asm("hlt":::);
            while(1);
        }else if(strncmp("clear", input, 5) == 0){
            clrscrn();
            continue;
        }else if(strncmp("uptime", input, 6) == 0){
            pint(get_uptime());
        }else if(strncmp("getscan", input, 7) == 0){
            pstr("scan code: ");
            pint((int)get_scan());
        }else if(strncmp("echo ", input, 5) == 0){
            pstr(input);
            newline();
            for(unsigned short i = 5; i < sizeof(input); i++){ //fix (doesnt print the rest but knows it)
                pchar(input[i]); //doesnt print anything visible
            }
            newline();
